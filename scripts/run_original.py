...
from simulation_tool import templates
...
def prep_random_simulation(index, manual_randomization=False):
    dpi = 60
    session_path_old = PATH_TO_SIMSS
    ...
    print(f"Bandgap: {bandgap_nm} nm, {bandgap_ev} eV")
    ...
    if manual_randomization:
        ...
        layer1_config = templates.get_layer1_config(
            session_path, session_path_old, index
        )
        layer2_config = templates.get_layer2_config(
            session_path, session_path_old, index, bandgap_ev
        )
        layer3_config = templates.get_layer3_config(
            session_path, session_path_old, index
        )
        simss_config_text = templates.get_simss_config_text(simss_config)
        layer1_config_text = templates.get_layer_config_text(
            extract_lx(layer1_config, "l1")
        )
        layer2_config_text = templates.get_layer_config_text(
            extract_lx(layer2_config, "l2")
        )
        layer3_config_text = templates.get_layer_config_text(
            extract_lx(layer3_config, "l3")
        )
        randomized_settings = {}
        ...
        randomized_settings = templates.sample_random_parameters(
            session_path, session_path_old, index, bandgap_ev
        )
        simss_config_text = templates.get_simss_config_text(randomized_settings)
        layer1_config_text = templates.get_layer_config_text(
            extract_lx(randomized_settings, "l1")
        )
        layer2_config_text = templates.get_layer_config_text(
            extract_lx(randomized_settings, "l2")
        )
        layer3_config_text = templates.get_layer_config_text(
            extract_lx(randomized_settings, "l3")
        )

    simss_device_parameters_filename = (
        f"{session_path}/simulation_setup_custom_{index}.txt"
    )
...
def run_simulation(index, session_path, do_EQE=True):
    ...
    device_characteristics_filename_new = (
        f"{session_path}/device_characteristics_{index}.txt"
    )
    if os.path.exists(device_characteristics_filename_old):
        os.system(
            f"mv {device_characteristics_filename_old} {device_characteristics_filename_new}"
        )
        data2 = [
            float(x)
            for x in open(device_characteristics_filename_new, "r")
            .readlines()[1]
            .split()
        ]
